...
incidence_target = pd.read_csv(
    os.path.join(TARGET_DIR, "crc_incidence_1y.csv"), index_col=0, header=[0, 1]
)
...
stage_dist_target = pd.read_csv(
    os.path.join(TARGET_DIR, "stage_distribution.csv"), index_col=0
)
...
polyp_target = pd.read_csv(
    os.path.join(TARGET_DIR, "adenoma_risk_age60.csv"), index_col=0
)
