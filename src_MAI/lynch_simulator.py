...
def unif_dist(df):
    for i in range(df.shape[1]):
        ...
        if col_val[0] == 0:
            ...
            val_list = []
            first_val = np.random.uniform(col_val[0] * 0.5, col_val[0])
            val_list.append(first_val)
            for j in range(len(col_val) - 1):
                if col_val[j + 1] != col_val[j]:
                    x = np.random.uniform(col_val[j] * 0.5, col_val[j])
...
def get_start_state(states, run_spec):
    ...
    if run_spec.interval == 0 or run_spec.start_age != ps.START_AGE:
        start_state[0][3] = 1  # start in NH
    elif run_spec.interval == 1:
        start_state[0][1] = 1  # start in
    else:
        start_state[0][2] = 1
    return start_state
...
def create_t_matrix(run_spec, time=ps.time, age_0=ps.START_AGE):
    ...
    # assigns gender-specific params, e.g. utility, mortality
    # currently model is only configured for "both"
    this_gender = g.gender_obj(run_spec.gender)

    # If we aren't screening in the first year, but we are in the first year,
    # create temp run_spec object to set adenoma transtion == 0
    # ? How does this set adenoma transition to 0? Sets surv interval to 0
    ...
    # dict_to_connect_matrix -> matrix with 1s where people can transition
    # choose_guidelines ->
    c_matrix = choose_guidelines(dict_to_connect_matrix(states, connect), this_run_spec)
    ...
    all_cause_dx_states = [
        names["init dx stage I"],
        names["init dx stage II"],
        names["init dx stage III"],
        names["init dx stage IV"],
        names["dx stage I"],
        names["dx stage II"],
        names["dx stage III"],
        names["dx stage IV"],
    ...
    nodes_nono, risk_probs_nono, risk_rates_nono = pf.cumul_prob_to_annual(
        this_gender.params, this_run_spec.gene, 1
    )

    if type(this_run_spec.risk_ratio) != str:
        nodes, risk_probs, risk_rates = pf.cumul_prob_to_annual(
            this_gender.params, this_run_spec.gene, this_run_spec.risk_ratio
        )

        nodes_adn, risk_adn_list, adn_rates = pf.cumul_prob_KM(ps.risk_adn_male_data)
    #        nodes_adn_2, risk_adn_list_2, adn_rates_2 = pf.cumul_prob_KM(ps.risk_adn_female_data)

    #        nodes_adn = [(x+y)/2 for (x, y) in zip(nodes_adn_1, nodes_adn_2)]
    #        risk_adn_list = [(x+y)/2 for (x, y) in zip(risk_adn_list_1, risk_adn_list_2)]
    #        adn_rates = [(x+y)/2 for (x, y) in zip(adn_rates_1, adn_rates_2)]
    ...
    for t in time:
        ...
        else:  # No asymptomatic screening
            csy_death_states = [
                names["init adenoma"],
                names["adenoma"],
                names["init adv adenoma"],
        ...
        if type(this_run_spec.risk_ratio) != str:
            ...
            adv_adn_risk_list = pf.annual_rate_to_prob(
                risk_rates * (ps.adv_adenoma_risk_mult), nodes
            )
            adv_adn_risk = pf.pw_choose_prob(age, adv_adn_risk_list, nodes)
            adn_dx_risk = pf.weighted_avg(
                [adv_adn_risk, dx_risk_prob],
        ...
        stage_2_death, stage_3_death, stage_4_death = pf.get_cancer_death_probs(
            age, this_gender
        )
        ...
        if t == 0:
            ...
            temp[names["adenoma"], names["init dx stage I"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_1"]
            )
            temp[names["adenoma"], names["init dx stage II"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_2"]
            )
            temp[names["adenoma"], names["init dx stage III"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_3"]
            )
            temp[names["adenoma"], names["init dx stage IV"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_4"]
            )

            temp[names["init adenoma"], names["init dx stage I"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_1"]
            )
            temp[names["init adenoma"], names["init dx stage II"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_2"]
            )
            temp[names["init adenoma"], names["init dx stage III"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_3"]
            )
            temp[names["init adenoma"], names["init dx stage IV"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_4"]
            )
            ...
            # =============================================================================
            temp[names["adv adenoma"], names["init dx stage I"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_1"]
            )
            temp[names["adv adenoma"], names["init dx stage II"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_2"]
            )
            temp[names["adenoma"], names["init dx stage III"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_3"]
            )
            temp[names["adv adenoma"], names["init dx stage IV"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_4"]
            )

            temp[names["init adv adenoma"], names["init dx stage I"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_1"]
            )
            temp[names["init adv adenoma"], names["init dx stage II"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_2"]
            )
            temp[names["init adv adenoma"], names["init dx stage III"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_3"]
            )
            temp[names["init adv adenoma"], names["init dx stage IV"]] = (
                adn_dx_risk * ps.staging.loc[this_run_spec.interval, "stage_4"]
            )

            ##probabilities for normal -> dx
            temp[names[this_run_spec.guidelines], names["init dx stage I"]] = (
                dx_risk_prob * ps.staging.loc[this_run_spec.interval, "stage_1"]
            )
            temp[names[this_run_spec.guidelines], names["init dx stage II"]] = (
                dx_risk_prob * ps.staging.loc[this_run_spec.interval, "stage_2"]
            )
            temp[names[this_run_spec.guidelines], names["init dx stage III"]] = (
                dx_risk_prob * ps.staging.loc[this_run_spec.interval, "stage_3"]
            )
            temp[names[this_run_spec.guidelines], names["init dx stage IV"]] = (
                dx_risk_prob * ps.staging.loc[this_run_spec.interval, "stage_4"]
            )
            ...
            if (
                ps.CRC_death_rate.loc[2, "death_rate"]
                > this_gender.lynch_ac_mortality[age]
            ):
                temp[names["dx stage II"], names["stage II death"]] = stage_2_death
                temp[names["init dx stage II"], names["stage II death"]] = stage_2_death
            if (
                ps.CRC_death_rate.loc[3, "death_rate"]
                > this_gender.lynch_ac_mortality[age]
            ):
                temp[names["dx stage III"], names["stage III death"]] = stage_3_death
                temp[names["init dx stage III"], names["stage III death"]] = (
                    stage_3_death
                )
            if (
                ps.CRC_death_rate.loc[4, "death_rate"]
                > this_gender.lynch_ac_mortality[age]
            ):
            ...
            #             CHANGE THIS PROB
            # =============================================================================
            temp[names["adenoma"], names["init adv adenoma"]] = (
                1 - this_gender.lynch_ac_mortality[age] - adn_dx_risk - csy_death_risk
            )
            ...
            temp[names["init dx stage I"], names["dx stage I"]] = (
                1 - this_gender.lynch_ac_mortality[age] - colectomy_death
            )
            temp[names["init dx stage II"], names["dx stage II"]] = (
                1 - this_gender.lynch_ac_mortality[age] - stage_2_death
            )
            temp[names["init dx stage III"], names["dx stage III"]] = (
                1 - this_gender.lynch_ac_mortality[age] - stage_3_death
            )
            temp[names["init dx stage IV"], names["dx stage IV"]] = (
                1 - this_gender.lynch_ac_mortality[age] - stage_4_death
            )
            temp[names["init adenoma"], names["adenoma"]] = (
                1 - this_gender.lynch_ac_mortality[age] - adn_dx_risk - csy_death_risk
            )

        # normalizes cells such that each cell = 1- sum(all other cells)
        for row in range(14):
            pf.normalize_target(temp[row], row)
            # print(sum(temp[row]))
...
def create_t_matrix_PSA(run_spec, time=ps.time, age_0=ps.START_AGE):
    ...
    for t in time:
        ...
        if age < 60:
            colectomy_death = ps.colectomy_death_risk[0]
            colectomy_death = colectomy_death * np.random.uniform(0.95, 1.05)
        elif age < 70:
            colectomy_death = ps.colectomy_death_risk[1]
            colectomy_death = colectomy_death * np.random.uniform(0.95, 1.05)
        else:
        ...
        stage_2_death = stage_2_death * np.random.uniform(0.95, 1.05)
        stage_3_death = stage_3_death * np.random.uniform(0.95, 1.05)
        stage_4_death = stage_4_death * np.random.uniform(0.95, 1.05)
...
def av_matrix(run_spec):
    """Takes in information about a run, and creates transition matrices to be used in Markov model.
    Importantly makes distinction for allowing transition into colonoscopy/surveillance states

    Args:
        run_spec (run_type object): Instance of run_type class specifying gene, surveillance interval, age of surveillance start, gender, etc.

    Returns:
        matrix: Transition matrix for Markov model
    """
    ...
    if run_spec.start_age == ps.START_AGE:
        ...
        # Create matrix for first end_age_1 years, without possibility of transition to colonoscopy surveillance
        t_matrix_1 = create_t_matrix(run_spec, time=time_1, age_0=age_start)
        # Create matrix for remaining end_age_2 years, with possibility of surveillance state transitions
        t_matrix_2 = create_t_matrix(run_spec, time=time_2, age_0=run_spec.start_age)
        # Append matrices to get full years of model run (should be shape (num_ages, num_health_states, num_health_state))
...
def av_matrix_PSA(run_spec):
    ...
    if run_spec.start_age == ps.START_AGE:
        ...
        t_matrix_2 = create_t_matrix_PSA(
            run_spec, time=time_2, age_0=run_spec.start_age
        )
        t_matrix = np.vstack((t_matrix_1, t_matrix_2))
        # csy_checker = np.append(csy_checker_1, csy_checker_2)
...
def owsa_t_matrix(t_matrix, s_1, s_2, multiplier):
    ...
    t_matrix[:, names[s_1], names[s_2]] = (
        t_matrix[:, names[s_1], names[s_2]] * multiplier
    )
...
def run_markov_simple(run_spec, t_matrix):
    ...
    states = ps.ALL_STATES  # idx to state
    names = dm.flip(states)  # state to idx
    age_0 = ps.START_AGE  # start age for simulation
    time = ps.time  # array [0, ..., 49]

    # start_state is one-hot vector shape (1, num_states) with one-hot in idx 1,2,3
    # corresponding to current, new, nono. If surveillance start_age != 25, start_state
    # is "nono" until start_age, then switches to surveillance guidelines in for-loop
    start_state = get_start_state(states, run_spec)

    # creates a DataFrame for the population in each state for each time point
    # Beginning df has only one row with value 1 in start_state, 0s elsewhere
    D_matrix = pd.DataFrame(start_state, columns=states)

    # Initialize logs
    cancer_incidence = np.zeros(51)
    overall_survival = np.zeros(51)
    overall_survival[0] = 1  # Everyone alive at stage 0
    age = np.zeros(51)
    age[0] = ps.START_AGE

    # Create array to assign True to appropriate surveillance years
    # If "nono", populate csy_tracker with False for all years
    if run_spec.interval == 0:
        csy_tracker = np.full(51, False)
    else:
        csy_tracker = np.full(51, False)
    ...
    cancer_states = [
        ...
        names["cancer death"],
        names["stage I death"],
        names["stage II death"],
        names["stage III death"],
    ...
    for t in time:
        ...
        # Start surveillance
            ...
            # print(Distribution)
            # Transfer population from "nono" state to appropriate surv state
            temp = Distribution
            Distribution[names[run_spec.guidelines]] = temp[3]
            Distribution[3] = 0

        # Apply tmat for time t, get new population distribution
        temp = np.transpose(t_matrix[t]) * Distribution
        Distribution = [sum(temp[i, :]) for i in range(len(states))]

        # Incident deaths, overall surv, cancer inc
        # ? We don't do anything with these, but could save as csv
        age[t + 1] = age[0] + t + 1
        death_temp = np.sum(Distribution[i] for i in death_states)
        overall_survival[t + 1] = 1 - death_temp
        cancer_incidence[t + 1] = np.sum(Distribution[i] for i in cancer_states)

        # Append distribution for time t to D_matrix
    ...
    # Finalize D_matrix, add other outputs
    # Rename cols to match state names
    # Add additional cols for cancer inc, overal surv, age, n csy
...
def run_markov_MCLIR(run_spec, MCLIR_age):
    #    print('In run_markov_simple..')
    states = ps.ALL_STATES
    age_0 = ps.START_AGE
    ...
    for t in time:
        ...
        if t == MCLIR_age:
            Distribution[names[run_spec.guidelines]] = Distribution[
                names[run_spec.guidelines]
            ] + sum(Distribution[4:])
            Distribution[4:] = [0] * len(Distribution[4:])
...
def CRC_results_by_interval(
    genes=ps.genes,
    genders=ps.genders,
    intervals=ps.intervals,
    csy_start_age=ps.START_AGE,
    ...
    for gender in genders:
        ...
        for gene in genes:
            ...
            for i in intervals:
                run_spec = ps.run_type(
                    i, gene, this_gender.gender, age_spec=csy_start_age
                )
                t_matrix = av_matrix(run_spec)
            ...
            plt.figure()
            plt.title(
                "CRC Incidence, " + gene + ", " + "Start age: " + str(csy_start_age)
            )
            plt.xlabel("Age")
...
def CRC_MCLIR_results_by_interval(
    genes=ps.genes,
    genders=ps.genders,
    intervals=ps.intervals,
    csy_start_age=ps.START_AGE,
    ...
    for gender in genders:
        ...
        for gene in genes:
            ...
            for i in intervals:
                ...
                D_matrix, t_matrix = run_markov_MCLIR(run_spec)
...
def OS_results_by_interval(
    ...
    for gender in genders:
        ...
        for gene in genes:
            ...
            plot_label = (
                "Overall Survival, " + gene + ", " + "Start age: " + str(csy_start_age)
            )
            plt.figure()
    ...
    for gender in genders:
        ...
        for gene in genes:
            ...
            for i in intervals:
                ...
                ac_df[run_spec.interval_str] = (
                    D_temp["all cause dx"] + D_temp["all cause"]
                )
...
def graph_everything_by_int(
    genes_every=ps.genes,
    genders_every=ps.genders,
    intervals_every=ps.intervals,
    start_ages=[ps.START_AGE],
    overwrite_pngs=False,
):

    for i in start_ages:

        CRC_results_by_interval(
            genes=genes_every,
            genders=genders_every,
            intervals=intervals_every,
            csy_start_age=i,
            overwrite_file=overwrite_pngs,
        )
        OS_results_by_interval(
            genes=genes_every,
            genders=genders_every,
            intervals=intervals_every,
            csy_start_age=i,
...
def CD_optimal_only():
    ...
    for gene in genes:
        cancer_df = pd.DataFrame()
        these_strats = ps.all_optimal_strats
        k = 0
        ...
        ax_array[i].set_ylim(bottom=0, top=0.14)
...
def CD_natural_history():
    ...
    for gene in genes:
        ...
        ymax = cancer_df[least_agro].max() + 0.01
        # file_name = 'CRC_incidence_'+ gene + '_'+ 'interval_constant_'+ str(interval) + '_combined_gender.png'
        ax_array[i].set_ylim(bottom=0, top=0.15)
        ax_array[i].set_xlim(left=25, right=75)
        i += 1
    plt.savefig(
        ps.cd_results / "optimal_strats_nat_hist.png", bbox_inches="tight", dpi=500
    )


def CD_MCLIR_natural_history(MCLIR_age):
    genes = ps.genes
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(12, 12))
    ax_array = [ax1, ax2, ax3, ax4]
    plt.suptitle(
        "Natural History Incidence Rate Vs. Perfect Intervation Rate",
        fontsize=14,
        y=0.94,
    )
    i = 0
    for gene in genes:
        ...
        for k in range(0, len(these_strats)):
            ...
            print(interval, age_start)
            run_spec = ps.run_type(interval, gene, ps.genders[0], age_spec=age_start)
            D_matrix, t_mat = run_markov_MCLIR(run_spec, MCLIR_age)
            cancer_df[run_spec.label] = D_matrix.loc[1:, "cancer death"]
            if k == 0:
                least_agro = run_spec.label
        ax_array[i].set_title(gene)
        ...
        i += 1
    plt.savefig(
        ps.cd_results / "optimal_strats_" + str(MCLIR_age) + "_perf_intervention.png",
        bbox_inches="tight",
        dpi=500,
    )
...
def CRC_optimal_only():
    ...
    for gene in genes:
        ...
        ymax = cancer_df[least_agro].max() + 0.01
        # file_name = 'CRC_incidence_'+ gene + '_'+ 'interval_constant_'+ str(interval) + '_combined_gender.png'
        ax_array[i].set_ylim(bottom=0, top=0.55)
        ax_array[i].set_xlim(left=25, right=75)
        i += 1
...
def CRC_natural_history():
    ...
    for gene in genes:
        ...
        ymax = cancer_df[least_agro].max() + 0.01
        # file_name = 'CRC_incidence_'+ gene + '_'+ 'interval_constant_'+ str(interval) + '_combined_gender.png'
        ax_array[i].set_ylim(bottom=0, top=0.6)
        ax_array[i].set_xlim(left=25, right=75)
        i += 1
    plt.savefig(
        ps.crc_results / "optimal_strats_nat_hist.png", bbox_inches="tight", dpi=500
    )
    return cancer_df


def CRC_MCLIR_natural_history(MCLIR_age):
    genes = ps.genes
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(12, 12))
    ax_array = [ax1, ax2, ax3, ax4]
    #    plt.suptitle('Natural History Incidence Rate Vs. Perfect Intervation Rate',
    #                 fontsize = 14, y = 0.94)
    #    plt.suptitle('Natural History Colon Cancer Incidence by Gene',
    #                 fontsize = 14, y = 0.94)
    i = 0
    for gene in genes:
        ...
        for col in cols:
            ax_array[i].plot(
                ps.age_time, cancer_df_m[col], "g", label="Perfect Intervention"
            )
            ax_array[i].plot(
                ps.age_time, cancer_df[col], "--", color="grey", label="Background Rate"
            )
        ax_array[i].legend()
        ymax = cancer_df[least_agro].max() + 0.01
        # file_name = 'CRC_incidence_'+ gene + '_'+ 'interval_constant_'+ str(interval) + '_combined_gender.png'
        ...
        ax_array[i].set_xlim(left=25, right=75)
        i += 1
    filename = "optimal_strats_" + str(MCLIR_age) + "_perf_intervention.png"
    plt.savefig(ps.crc_results / filename, bbox_inches="tight", dpi=500)
    return cancer_df


def CRC_MCLIR_nat_hist_perc(MCLIR_age):
    genes = ps.genes
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(12, 12))
    ax_array = [ax1, ax2, ax3, ax4]
    #    plt.suptitle('Maximum Clinical Incidence Reduction (MCLIR)',
    #                 fontsize = 14, y = 0.94)
    #    plt.suptitle('Natural History Colon Cancer Incidence by Gene',
    #                 fontsize = 14, y = 0.94)
    i = 0
    for gene in genes:
        ...
        for k in range(0, len(these_strats)):
            ...
            if k == 0:
                least_agro = run_spec.label
        ax_array[i].set_title(gene)
        ax_array[i].set_ylabel("Incidence reduction as % of background incidence")
        ax_array[i].set_xlabel("Age")
        # plt.xlabel('Age')
        # plt.ylabel('CRC Incidence')
        # print(cancer_df.index.values)
        ...
        for col in cols:
            ax_array[i].plot(
                ps.age_time,
                (cancer_df[col] - cancer_df_m[col]) / cancer_df[col] * 100,
                "b",
                label="MCLIR",
            )
        ax_array[i].legend()
        ...
        ax_array[i].set_xlim(left=25, right=75)
        i += 1
    filename = "optimal_strats_" + str(MCLIR_age) + "_MCLIR.png"
    plt.savefig(ps.crc_results / filename, bbox_inches="tight", dpi=500)
    return cancer_df


# same as above, but with different starting ages. default interval is 1 year
def CRC_results_by_age(
    genes=ps.genes,
    genders=ps.genders,
    interval=0,
    csy_start_ages=ps.ages,
    overwrite_file=False,
):
    for gender in genders:
        this_gender = g.gender_obj(gender)
        cancer_df = pd.DataFrame()
        for gene in genes:
            run_spec = ps.run_type(interval, gene, this_gender.gender, age_spec=25)
            # this_gender = g.gender_obj(run_spec.gender)
            t_matrix = av_matrix(run_spec)
            D_matrix = run_markov_simple(run_spec, t_matrix)
    ...
    for gender in genders:
        ...
        for gene in genes:
            ...
            for i in csy_start_ages:
                run_spec = ps.run_type(interval, gene, this_gender.gender, age_spec=i)
                # this_gender = g.gender_obj(run_spec.gender)
                t_matrix = av_matrix(run_spec)
                D_matrix = run_markov_simple(run_spec, t_matrix)
            ...
            plt.figure()
            plt.title(
                "Cancer-Specific Death, " + gene + ", " + "Interval: " + str(interval)
            )
            plt.xlabel("Age")
            plt.ylabel("Cancer Mortality")
            cols = cd_df.columns
    ...
    for gender in genders:
        ...
        for gene in genes:
            ...
            for i in csy_start_ages:
                ...
                D_temp = D_matrix.iloc[1:, :]
                ac_df[str(run_spec.start_age)] = (
                    D_temp["all cause dx"] + D_temp["all cause"]
                )

            plt.figure()
            plt.title(
                "All cause mortality, " + gene + ", " + "Interval: " + str(interval)
            )
            plt.xlabel("Age")
            plt.ylabel("% Dead")
            cols = ac_df.columns
        ...
        for i in intervals:
            ...
            while k in range(0, len(ps.ages)):
                ...
                key_name = (
                    ...
                    + run_spec.gene
                    + "_"
                    + str(run_spec.interval)
                    + "_"
                    + str(run_spec.start_age)
                    + "_both_genders.csv"
                )
                ...
                D_matrix, t_matrix = run_markov_OWSA(
                    run_spec, state_1, state_2, multiplier
                )
...
def generate_output(gender, intervals=ps.intervals):
    """
    Runs Markov model for each gene, surveillance start age, and surveillance interval.

    Args:
        gender (string): male, female, both
        intervals (array, optional): Csy surveillance intervals to simulate. Defaults to ps.intervals.

    Returns:
        _type_: _description_
    """
    output_dict = {}
    # For each gene, MSH1, M,... PMS2
    for gene in ps.genes:
        # For each surveillance interval [1,2,3,4,5]
        for i in intervals:
            k = 0
            while k in range(0, len(ps.ages)):
                # if start age == 50, only test 5-year interval per current guidelines
                ...
                #                     i = 5
                # =============================================================================
                # Initialize run information, using surveillance interval, gene, gender, and current age
                run_spec = ps.run_type(i, gene, gender, age_spec=ps.ages[k])
                ...
                # Generate key name for use in output dictionary
                key_name = (
                    "D_matrix_"
                    + run_spec.gene
                    + "_"
                ...
                #                if pf.check_valid_file(file_name) == False or overwrite_file == True:
                #                    print('no valid file for run type, creating new one')
                # Passes run_spec instance thru av_matrix, to get transition matrix for cohort model for all years
                t_matrix = av_matrix(run_spec)

                # Run markov cohort model
                D_matrix = run_markov_simple(run_spec, t_matrix)
                ...
                #                                                    axis = 0)
                # =============================================================================
                # If in natural history (interval=0) and surveillance start age is 25, break loop and continue to next run_Spec
                if i == 0 and ps.ages[k] == 25:
                    k += 7
                else:
                    k += 1
...
def generate_output_lite_interval(
    genders=ps.genders, genes=ps.genes, age=ps.START_AGE, overwrite_file=False
):
    for gender in genders:
        this_gender = g.gender_obj(gender)
        for gene in genes:
...
def generate_output_lite_start_age(
    genders=ps.genders, genes=ps.genes, interval=1, overwrite_file=False
):
    for gender in genders:
        this_gender = g.gender_obj(gender)
        for gene in genes:
            ...
            for i in range(0, len(ps.ages)):
                run_spec = ps.run_type(
                    interval, gene, this_gender.gender, age_spec=ps.ages[i]
                )
                # this_gender = g.gender_obj(run_spec.gender)
...
def cumulative_sum_state(D_matrix, state):
    ...
    cml_vals_df = pd.DataFrame(
        {"age": range(ps.START_AGE, ps.START_AGE + len(cml_vals)), "cml_val": cml_vals}
    )
    return cml_vals_df
